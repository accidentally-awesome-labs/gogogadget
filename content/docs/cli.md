...
and the header was lost. Declaring the file in a module's `files` is **not** a
remedy and never was: `reconcilePlannedState` refuses any manifest target at a
generated path with "generated outputs are tool-owned and cannot be authored",
so declaring trades one exit 3 for another with the file still on disk.
