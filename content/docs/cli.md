...
- **A skip where nothing can be absent.** `go test` never summarises skips, so
  ...
  passed. `check` and `ggg test integration` read the `go test -json`
...
One exit 3 is easy to hit by accident: a file sitting at a name this pipeline
generates — `compose.yaml`, `.env.example`, `static/ui-components.js`, anything
with a `_registry_gen.` infix — that the selected graph does not render and
that carries no `Generated by ggg sync; DO NOT EDIT` header. `ggg` cannot prove
it wrote those bytes, so it refuses instead of sweeping them, names the path,
and names the three remedies that work: **delete** it, **rename** it to a name
the pipeline does not generate, or **restore the banner** if `ggg` did write it
and the header was lost — that last one returns the file to the swept class, so
the first run that does not render it names a `delete`/`generated` change and
**removes the bytes**. Declaring the file in a module's `files` is **not** a
remedy and never was: `reconcilePlannedState` refuses every payload at a
generated path with "generated outputs are tool-owned and cannot be authored",
and a payload that declares itself `class:"generated"` skips that guard only to
reach this same plan-time check against the **tree**, which reads the bytes and
never asks what class a manifest claims — so declaring trades one exit 3 for
another with the file still on disk.
`sync --claim` does not apply either — a claim adopts a divergent file against
a declared target, and declaring is the part that is refused.
The refusal rides `diagnostics[]` under `generated_unowned` as well as the
error text. See [the stale sweep](/docs/modules#stale-sweep).

Exit 4 has two distinct sources and both are honest reports rather than errors:

- `ggg sync --check` found pending changes or generated drift. That includes a
  pending **deletion**: a stale aggregate the tool wrote and no longer renders
  appears as a `delete`/`generated` change, path and digest and all, so the
...
When the rollback was triggered by a subprocess — every trusted task shells
out, and `ggg new` finishes by running `go mod tidy`, the generators and
`go build ./...` — the error carries the **tail of what that child printed**,
up to twenty lines, alongside its exit status, and `diagnostics[]` carries the
same text under `command_failed`. It is not optional detail: a rolled-back
`ggg new` has already removed the tree, so `go mod tidy: exit status 1;
nothing was kept, fix the cause and re-run` told an operator to fix a cause the
tool had just thrown away. The tail passes through the same secret redactor as
