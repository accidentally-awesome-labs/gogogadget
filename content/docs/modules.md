...
plays no part in detecting generated drift: `GeneratedDrift` renders the
aggregates and compares them byte-for-byte with the tree, emitting
`generated_missing` or `generated_drift`. It never reads `base_sha256`.

One code left that set. `generated_stale` — "generated output is no longer
owned by the selected graph; run ggg sync to delete it" — **no longer exists**.
The fact it carried now arrives as a plan change instead: a `delete` of class
`generated`, with the path and the digest of the bytes to be removed. A machine
consumer that filtered `diagnostics[]` on `generated_stale` must move to
`changes[] where kind == "delete" && class == "generated"`, or it will read a
stale aggregate as a clean tree.
...
### The stale sweep, and the two things it can find {#stale-sweep}

`ggg sync` deletes the outputs the selected graph no longer renders. Several
emitters produce nothing at all once their input set empties, so removing the
last module that declared a scenario, an API operation, or a locale used to
leave an aggregate on disk that still compiled into the build and still
referenced renderers the removal had deleted.

The set the sweep considers at all is `IsRegistryOwnedOutputPath` in
...
A name is not authorship, though. `compose.yaml` is a registry-owned name in
every project, generated or not, and a project that does not install
`ggg/system/docker` renders nothing there — so the deciding question is not
"is this one of our names" but "did **we** write this". Every file this
pipeline emits carries `Generated by ggg sync; DO NOT EDIT` in its header
(`modkit.GeneratedOutputMarker`, enforced on every emitted file by
`GenerateAll`), and the planner reads the bytes:

| Unrendered file at a registry-owned name | Verdict |
|---|---|
| carries the marker | a stale aggregate. It becomes a named `delete`/`generated` change in the plan, printed by `sync`, carried in `changes[]` under `--json`, journalled, and rolled back with everything else |
| does not carry the marker | authored bytes. Every command **refuses**, exit 3, naming the path and the remedies that work: delete it, rename it off the generated name, or restore the banner if `ggg` wrote it |

So no byte leaves the tree without being named, and nothing this tool cannot
prove it wrote is ever removed. The refusal is the same verdict
`ValidateRegistryTreeOwnership` reaches for the same question about the
registry tree, and it is why `sync --check`'s exit code now distinguishes the
two cases: exit 3 is "would delete bytes ggg did not write", exit 4 is "would
write or delete generated output".

**Declaring the file in a module's `files` is not a remedy**, although the
refusal used to say it was. It cannot work for any path the refusal can print:
the set is filtered by `IsRegistryOwnedOutputPath`, `IsGeneratedOutputPath` is
that predicate ORed with the external-tool outputs, and `reconcilePlannedState`
refuses every payload whose target satisfies it —
"generated outputs are tool-owned and cannot be authored" — before any
lock-state branching. Declaring therefore trades one exit 3 for another and
leaves the file exactly where it was. `sync --claim` does not reach it either:
a claim adopts a divergent file against a *declared* target, and declaring is
the part that is refused. The three working moves are the ones in the table —
`rm` it, `git mv` it to a name outside `IsRegistryOwnedOutputPath`, or put the
banner back if this really is an older `ggg` output.

**What the refusal can and cannot fire on** is what makes genesis and the
upgrade path safe, and it is narrower than it first reads: the gate only sees a
registry-owned name the graph does **not** render. A rendered path is compared
by content and never classified at all. `emitEnvExample` and
`emitComposeRegistry` are unconditional emitters — they write their banner on
every run, for every graph — so `.env.example`, `compose.yaml` and
`compose.test.yaml` are *always* rendered, and an adopted hand-written one
**drifts** (exit 4, re-rendered clean) rather than refusing. Measured on both
ends: a fresh `ggg new --profile ggg/profile/saas` exits 0 on its first
...
render stops producing it. That follows from the rule this whole layer rests on
— generated files are never hand-edited, and every one of them says so in its
first line — and the marker is the claim of authorship, so deleting the banner
is how you withdraw it. `generated` in the `--json` envelope lists the outputs
a run **wrote**; a deletion appears only in `changes[]`, where its kind and the
digest of the removed bytes are unambiguous.
