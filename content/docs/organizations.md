...
**Settings → Organization → Export data** (`org:admin` only) enqueues an
`export.org_json` job. The worker writes one JSON document through the storage
seam, records it on `/app/files`, and sends the requester an `export.ready`
notification with the download link — the same path the projects CSV takes.

It contains the organization row, members and roles, projects, file metadata,
the audit log, billing state, and the API-token and webhook inventory.

**Secrets are excluded by construction.** Rows are mapped to explicit DTOs
rather than marshaled from sqlc, because `api_tokens.token_hash`,
`webhook_endpoints.secret` and `secret_previous` all carry json tags: encoding
the rows would write live signing secrets and token hashes into a file the
customer downloads and forwards. A test asserts the rendered document contains
neither, and the export states the omission in a `notice` field so the
recipient is not left guessing. Storage keys are omitted too — they are
internal addresses, not customer data.

Each collection is capped at 10,000 rows and the document carries a
`truncated` map, so a large organization gets an honest partial snapshot
rather than a silent one.

Why a job rather than an inline download like the per-user GDPR export: this
one spans every member's activity and the whole audit trail, which is not a
size that belongs on a request thread.

Why `org:admin`: the per-user export next door contains only the caller's own
data and stays open to everyone; this is an owner's view of the company.
