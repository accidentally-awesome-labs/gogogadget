...
- **Design system is three layers, one home each**, and templates consume only the top two: **tokens** (`input.css` `@theme` + `.dark`; `theme.go` for email, the one surface that inlines hex), **component classes** (`input.css` `@layer components` — `.btn`/`.btn-{primary,ghost,danger,inverse}` × `.btn-{sm,xs,icon}`, `.badge-*`, `.alert-*`, `.banner-*`, `.toast-*`, `.input`, `.card`, `.table-card`, `.page-*`, `.nav-link`, `.tab`, `.prose`), **templ components** (`components.templ` + `icons.templ` — `@PageHeader`, `@TableCard`, `@Pagination`, `@SearchToolbar`, `@Badge`/`@Alert`/`@Banner` over the shared `Kind` axis, `@Icon`). `designsystem_test.go` fails the build on a raw hex, `dark:` variant, palette ramp, numeric brand step, `!` override, arbitrary length, or templ expression inside a quoted attribute — **zero exemptions**. Dark mode is token flipping, never a `dark:` class. Chrome identity/navigation is data in `chrome.go` (`BrandName`, `PublicNav`, `AppNav`, `AdminNav`, `FooterColumns`, `DocsEditBase`). `/dev/gallery` renders every token and component in every variant (dev-only route, one visual baseline + axe sweep).
