...
- **Middleware order is load-bearing**: recover → requestID → accessLog → i18n.Detect → maintenanceMode → rateLimit → secureHeaders → sessionLoad → csrf → route groups (app: `requireAuth → requireNotDisabled → requireOrg → loadPlan`; admin: that chain + `requireStaff → requireAdminWrite`; `/api`: `RequireAPIToken`).
