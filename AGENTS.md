...
- **One options struct per UI component**: every exported renderer in `package ui` is `templ Name(o NameOpts)` and every `NameOpts` embeds `ui.Attrs` (`ID`, `Class`, `TestID`, `Title`, `Decorative`, `Data`, CSP-safe `Alpine`, `HX`). `Attrs` has NO arbitrary-attribute map — components own `role`/`aria-*`/`tabindex`/`type`/base class and callers cannot override them. Enums are closed and normalize (empty or unknown → `KindNeutral`, `SizeMD`, `LiveOff`, …). `ui` imports templ + stdlib only, never `templates`/`billing`/`identity`/sqlc. Held by `ui/contract_test.go`; the 174 signatures live in `ui/reference_gen.go` and `ggg info`, never in prose.
