...
func New(baseURL string) *Client {
	return &Client{BaseURL: baseURL, confirmed: make(map[string]string)}
}
...
func (c *Client) RevokeSubscription(_ context.Context, sub string) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	delete(c.confirmed, sub)
	return nil
}
func (c *Client) IngestUsage(context.Context, string, []billing.UsageEvent) error { return nil }
func (c *Client) Confirm(product, customer string) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.confirmed[customer] = product
}
func (c *Client) Cancel(customer string) {
	c.mu.Lock()
	defer c.mu.Unlock()
