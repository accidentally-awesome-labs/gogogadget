...
func NewClerkDeleter(secretKey string) Deleter {
	return &clerkDeleter{client: user.NewClient(&clerk.ClientConfig{BackendConfig: clerk.BackendConfig{Key: &secretKey}})}
}
func (d *clerkDeleter) DeleteUser(ctx context.Context, userSubject string) error {
