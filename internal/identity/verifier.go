...
type Verifier interface { Verify(context.Context, string) (*ProviderClaims, error) }
...
func (FakeVerifier) Verify(_ context.Context, token string) (*ProviderClaims, error) {
  ...
  if len(parts) != 4 || parts[0] != "e2e" || parts[1] == "" { return nil, fmt.Errorf("%w: want e2e:<userID>:<orgID>:<role>", ErrInvalidToken) }
