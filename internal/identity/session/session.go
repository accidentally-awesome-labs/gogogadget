...
func (l *SessionLoader) Load(ctx context.Context, token string) (Session, error) {
	if l == nil || l.Pool == nil || l.Verify == nil || l.Fetch == nil {
		return Session{}, errors.New("identity-session: dependencies are required")
	}
	pc, err := l.Verify.Verify(ctx, token)
	if err != nil {
		return Session{}, err
	}
	if pc == nil || pc.Provider == "" || pc.UserSubject == "" {
		return Session{}, errors.New("identity-session: verifier returned incomplete claims")
	}
	...
	var user sqlc.User
	if err == nil {
		user, err = q.GetUserByID(ctx, mapped.UserID)
		if err != nil {
			return Session{}, err
		}
		...
		profile, fetchErr := l.Fetch.Fetch(ctx, pc.UserSubject)
		if fetchErr != nil {
			return Session{}, fetchErr
		}
		// Mutable profile data never links to an existing user implicitly.
		if _, emailErr := q.GetUserByEmail(ctx, profile.Email); emailErr == nil {
			return Session{}, identity.ErrLinkRequired
		} else if !errors.Is(emailErr, pgx.ErrNoRows) {
			return Session{}, emailErr
		}
		userID, genErr := opaqueID("usr_")
		if genErr != nil {
			return Session{}, genErr
		}
		user, err = q.UpsertUser(ctx, sqlc.UpsertUserParams{UserID: userID, Email: profile.Email, Name: profile.Name, AvatarUrl: profile.AvatarURL})
		if err != nil {
			return Session{}, err
		}
	...
	if pc.OrgSubject != "" {
		...
		if orgErr == nil {
			o, loadErr := q.GetOrgByID(ctx, om.OrgID)
			if loadErr != nil {
			...
			o, upErr := q.UpsertOrg(ctx, sqlc.UpsertOrgParams{OrgID: orgID, Name: pc.OrgSlug, Slug: pc.OrgSlug, ImageUrl: ""})
			if upErr != nil {
				return Session{}, upErr
			}
			if _, upErr = q.InsertIdentityOrganization(ctx, sqlc.InsertIdentityOrganizationParams{Provider: pc.Provider, Subject: pc.OrgSubject, OrgID: o.OrgID}); upErr != nil {
				return Session{}, upErr
			}
			if upErr = q.UpsertMembership(ctx, sqlc.UpsertMembershipParams{OrgID: o.OrgID, UserID: user.UserID, Role: pc.OrgRole}); upErr != nil {
				return Session{}, upErr
			}
			org = &o
		}
...
func opaqueID(prefix string) (string, error) {
	...
	if _, err := rand.Read(b[:]); err != nil {
		return "", fmt.Errorf("identity-session: generate id: %w", err)
	}
