...
// A project file written before `ports` existed — every project generated by
// an earlier CLI, including every one created from a published release — has
// no `ports` key at all. That is data at rest an upgrade cannot rewrite, so
// requiring the key would refuse the very next command in an existing project
// under an unchanged schema version. The mutation that proves it is deleting
// the key from a canonical project file, exactly the shape an older writer
// produced.
func TestParseProjectAcceptsAnAbsentPortsKeyAndNeverWritesOne(t *testing.T) {
	legacy := strings.Replace(canonicalProjectJSON, "  \"ports\": {},\n", "", 1)
	if strings.Contains(legacy, "ports") {
		t.Fatal("mutation did not remove the ports key")
	}
	absent, err := ParseProject([]byte(legacy))
	if err != nil {
		t.Fatalf("ParseProject(no ports key): %v", err)
	}
	if len(absent.Ports) != 0 {
		t.Fatalf("absent ports parsed as %#v", absent.Ports)
	}
	empty, err := ParseProject([]byte(canonicalProjectJSON))
	if err != nil {
		t.Fatalf("ParseProject(empty ports): %v", err)
	}
	// An absent map and an empty one are the same intent, and writing either
	// back must not add a key an older reader has never seen.
	for _, project := range []Project{absent, empty} {
		data, marshalErr := MarshalProject(project)
		if marshalErr != nil {
			t.Fatalf("MarshalProject: %v", marshalErr)
		}
		if strings.Contains(string(data), "ports") {
			t.Fatalf("marshalled project gained a ports key:\n%s", data)
		}
	}
	// A declared override still round-trips.
	empty.Ports = map[string]PortOverrides{"app/http": {Development: 8081}}
	data, err := MarshalProject(empty)
	if err != nil {
		t.Fatalf("MarshalProject(override): %v", err)
	}
	if !strings.Contains(string(data), `"app/http"`) || !strings.Contains(string(data), `"development": 8081`) {
		t.Fatalf("declared override did not round-trip:\n%s", data)
	}
	reparsed, err := ParseProject(data)
	if err != nil {
		t.Fatalf("ParseProject(override): %v", err)
	}
	if reparsed.Ports["app/http"].Development != 8081 {
		t.Fatalf("reparsed ports = %#v", reparsed.Ports)
	}
}
