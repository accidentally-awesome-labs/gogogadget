...
func validateLockedFile(file LockedFile) error {
	...
	if !validSHA256(file.BaseSHA256) {
	...
	case FileMissing:
		if file.LocalSHA256 != "" {
			return fmt.Errorf("local_sha256 for missing state must be empty")
		}
	}
	return nil
}
...
func validFileState(value FileState) bool {
	switch value {
	case FileClean, FileModified, FileMissing, FileConflicted:
		return true
	default:
		return false
	}
...
func validFileClass(value FileClass) bool {
	switch value {
	case FileClassGo, FileClassTempl, FileClassStyle, FileClassScript, FileClassAsset, FileClassQuery,
