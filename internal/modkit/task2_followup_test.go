...
func TestExtractToolRejectsZipTraversalAndUnlistedExecutables(t *testing.T) {
	...
	for _, name := range []string{"tool", "extra"} {
...
func TestReconcileManagedDependenciesPreservesUserChange(t *testing.T) {
	...
	if _, err := ReconcileManagedDependencies(context.Background(), root, previous, nil, nil); err != nil { t.Fatal(err) }
	data, err := os.ReadFile(filepath.Join(root, "go.mod")); if err != nil { t.Fatal(err) }
	if !strings.Contains(string(data), "example.com/provider v1.4.0") { t.Fatalf("user requirement changed: %s", data) }
}
...
func TestRuntimeOrdersForAddsCapabilityEdges(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if indexOf(orders.Development, provider.ID) > indexOf(orders.Development, consumer.ID) { t.Fatalf("provider ordered after consumer: %v", orders.Development) }
}

func indexOf(items []string, want string) int { for i, item := range items { if item == want { return i } }; return len(items) }
func TestBootstrapBranchesByEnvironmentAndGatesAdapters(t *testing.T) {
	...
	for _, want := range []string{"switch r.Config.Env", "bootDevelopment", "bootTest", "bootProduction", "providerActive", "var _ apphost.HealthChecker"} {
		if !strings.Contains(out.Content, want) { t.Fatalf("bootstrap missing %q:\\n%s", want, out.Content) }
	}
	if strings.Count(out.Content, "local.New(ctx") != 2 || strings.Count(out.Content, "remote.New(ctx") != 1 { t.Fatalf("adapter constructors should be selected per environment: len=%d", len(out.Content)) }
}
func TestGeneratedExecutableContributionsCarryProviderGates(t *testing.T) {
	...
	routes, err := emitRoutesRegistry(context.Background(), "example.com/app", lock, graph); if err != nil { t.Fatal(err) }
	chrome, err := emitChromeRegistry(context.Background(), "example.com/app", lock, graph); if err != nil { t.Fatal(err) }
	slots, err := emitShellSlotsRegistry(context.Background(), "example.com/app", lock, graph); if err != nil { t.Fatal(err) }
	assets, err := emitStaticRegistry(context.Background(), "example.com/app", lock, graph); if err != nil { t.Fatal(err) }
	for _, content := range []string{routes.Content, chrome.Content, slots.Content, assets.Content} {
		if !strings.Contains(content, "providerActive") { t.Fatalf("executable contribution missing providerActive gate:\\n%s", content) }
	}
}
