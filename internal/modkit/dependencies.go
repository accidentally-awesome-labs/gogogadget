...
func reconcileGoMod(ctx context.Context, root string, previous, desired []LockedDependency, runner ToolRunner) ([]LockedDependency, error) {
	...
	if sumErr != nil && !os.IsNotExist(sumErr) { return nil, sumErr }
	...
	for _, dep := range desired {
		...
		if hadPrior {
			...
			if len(dep.Owners) == 0 { dep.Owners = append([]string{}, prior.Owners...) }
		} else if current != nil {
			dep.Preexisting = true
			dep.BaselineVersion = current.Mod.Version
		}
		if current == nil {
			if err := file.AddRequire(dep.Module, dep.ManagedVersion); err != nil { return nil, err }
	...
	for _, dep := range desired { desiredBy[dep.Module] = struct{}{} }
	for _, prior := range previous {
		if _, keep := desiredBy[prior.Module]; keep { continue }
		current := findRequire(file, prior.Module)
		if current == nil || current.Mod.Version != prior.ManagedVersion { continue }
		if prior.Preexisting {
			if prior.BaselineVersion == "" { continue }
			if err := file.AddRequire(prior.Module, prior.BaselineVersion); err != nil { return nil, err }
		} else {
	...
	if runner != nil {
		if err := runner.Run(ctx, root, []string{"go", "mod", "download"}); err != nil {
			...
			if sumErr == nil { _ = os.WriteFile(sumPath, oldSum, 0o644) } else { _ = os.Remove(sumPath) }
			return nil, err
		}
	}
	return next, nil
...
func findRequire(file *modfile.File, modulePath string) *modfile.Require {
	for _, req := range file.Require {
		if req.Mod.Path == modulePath { return req }
	}
	return nil
}
...
func ExtractTool(data []byte, artifact ToolArtifact, dst string) error {
	...
	case "zip":
		...
		for _, f := range z.File {
			if !validSafeArchivePath(filepath.ToSlash(f.Name)) { return fmt.Errorf("unsafe tool archive path %q", f.Name) }
			if f.Mode()&os.ModeSymlink != 0 { return fmt.Errorf("tool symlink rejected") }
			if f.Name == artifact.BinaryPath {
				if f.FileInfo().IsDir() { return fmt.Errorf("tool binary is a directory") }
				r, err := f.Open(); if err != nil { return err }
				payload, err = io.ReadAll(r); _ = r.Close(); if err != nil { return err }
				continue
			}
			if !f.FileInfo().IsDir() { return fmt.Errorf("undeclared tool executable %q", f.Name) }
		}
	case "tar.gz":
		gz, err := gzip.NewReader(bytes.NewReader(data)); if err != nil { return err }
		defer gz.Close()
		tr := tar.NewReader(gz)
		for {
			h, err := tr.Next()
			if err == io.EOF { break }
			if err != nil { return err }
			if !validSafeArchivePath(filepath.ToSlash(h.Name)) { return fmt.Errorf("unsafe tool archive path %q", h.Name) }
			if h.Name == artifact.BinaryPath {
				if h.Typeflag != tar.TypeReg { return fmt.Errorf("tool non-regular file rejected") }
				payload, err = io.ReadAll(tr); if err != nil { return err }
				continue
			}
			if h.Typeflag != tar.TypeDir { return fmt.Errorf("undeclared tool executable %q", h.Name) }
		}
	}
...
func ValidateDeclaredImports(files map[string][]byte, generated []string, declared []GoDependency) error {
	...
	scan := func(name string, data []byte) error {
		if !strings.HasSuffix(name, ".go") && !strings.HasSuffix(name, ".templ") { return nil }
		paths := []string{}
		...
		for _, path := range paths {
			if strings.HasPrefix(name, "generated_") && strings.HasPrefix(path, "golang.org/x/text/") { continue }
	...
	sort.Strings(names)
	for _, name := range names { if err := scan(name, files[name]); err != nil { return err } }
	for i, source := range generated { if err := scan(fmt.Sprintf("generated_%d.go", i), []byte(source)); err != nil { return err } }
	return nil
}
