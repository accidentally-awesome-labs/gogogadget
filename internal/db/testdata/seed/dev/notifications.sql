...
INSERT INTO notifications (org_id, user_id, kind, title, body, url, read_at)
SELECT v.org_id, v.user_id, v.kind, v.title, v.body, v.url, v.read_at
FROM (VALUES
...
) AS v(org_id, user_id, kind, title, body, url, read_at)
WHERE NOT EXISTS (
  SELECT 1 FROM notifications n
  WHERE n.user_id = v.user_id AND n.kind = v.kind AND n.title = v.title
);
