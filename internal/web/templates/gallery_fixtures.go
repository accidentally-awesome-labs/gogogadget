...
// Fixed demo inputs for the dev catalog's advanced-widget examples. These live
// with the gallery rather than beside the production adapters so removing
// page/dev-gallery takes them with it: a derivative that drops the catalog has
// no reason to carry a January 2026 calendar or a fake project table.
//
// Every value is fixed rather than derived from the clock or the database,
// because these renders are the visual baselines.

// dataTableToolbar builds the gallery's table toolbar. It is a function rather
// than inline markup because DataTable takes components as slots, and a slot
// argument cannot be written as templ markup at the call site.
func dataTableToolbar() templ.Component {
	return ui.TableToolbar(ui.TableToolbarOpts{Label: "Project table controls"})
}

// galleryChartBase builds a two-series fixture for the chart demos.
...
// galleryMonthGrid builds a fixed January 2026 for the demo.
