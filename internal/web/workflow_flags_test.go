...
// The admin flags surface is two modules: ggg/page/admin-flags serves and
// renders it, ggg/workflow/admin-flags mutates it. Every test here posts a
// mutation and three of them call s.invalidateFlagCache, which
// internal/web/workflow_admin_flags.go declares - so while they sat in the
// page module's payload, a closure that installed the page without the
// workflow (which every shipped profile does) wrote a tree whose tests did not
// compile.
