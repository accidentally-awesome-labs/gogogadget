...
func notifySeed(t *testing.T, s *Server, orgID, userID string) (unreadID int64) {
	...
	n1, err := s.q.InsertNotification(t.Context(), sqlc.InsertNotificationParams{
		OrgID: orgID, UserID: userID, Kind: "welcome", Title: "Welcome", Body: "body", Url: "/app",
	})
	require.NoError(t, err)
	_, err = s.q.InsertNotification(t.Context(), sqlc.InsertNotificationParams{
		OrgID: orgID, UserID: userID, Kind: "system", Title: "Old news", Body: "", Url: "",
	})
	require.NoError(t, err)
	// Third row already read.
	n3, err := s.q.InsertNotification(t.Context(), sqlc.InsertNotificationParams{
		OrgID: orgID, UserID: userID, Kind: "system", Title: "Read one", Body: "", Url: "",
	})
	require.NoError(t, err)
	require.NoError(t, s.q.MarkNotificationRead(t.Context(), sqlc.MarkNotificationReadParams{ID: n3.ID, OrgID: orgID, UserID: userID}))
	return n1.ID
}
...
func TestNotificationReadIsUserScoped(t *testing.T) {
	...
	n, err := s.q.CountUnreadByUser(t.Context(), sqlc.CountUnreadByUserParams{OrgID: "org_na", UserID: "user_na"})
	require.NoError(t, err)
